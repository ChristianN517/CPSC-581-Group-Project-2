...
export default function MobileProxyInner() {
    ...
    useEffect(() => {
        addLog("MobileProxyInner component mounted");
        return () => {
            addLog("MobileProxyInner component unmounting");
        };
    }, []);
    ...
    useEffect(() => {
        if (!code || !studentId) {
            setStatus("error");
            addLog("Missing code or studentId");
            return;
        }
        ...
        socket.on("connect", () => {
            ...
            socket.emit(
                ...
                (response: any) => {
                    console.log("📱 Proxy registration response:", response);
                    ...
                    if (navigator.vibrate) {
                        ...
                        console.log("📳 Link confirmation vibrate result:", result);
                        addLog(`Link confirmation vibrate result: ${result}`);
                    } else {
                        console.log("🔊 Link confirmation using audio beep");
        ...
        socket.on("haptic:fire", ({ pattern }) => {
            console.log("🔊 HAPTIC EVENT RECEIVED:", { pattern });
            addLog(`haptic:fire event received with pattern: ${pattern}`);
            ...
            if (navigator.vibrate) {
                ...
                console.log("🔊 Using audio beep fallback");
